...
// ==================== MODAL (Details) ====================
// ... (Modal Functions: loadEpisodes, createEpisodeListItem, showDetails, closeModal, changeServer - WALANG BINAGO DITO) ...
...
  const server = document.getElementById('server').value; let type = currentItem.media_type; if (!type) { type = currentItem.title ? 'movie' : 'tv'; }
  type = type === 'movie' ? 'movie' : 'tv'; const baseURL = CONFIG.SERVERS[server]; if (!baseURL) { console.error("Invalid server:", server); return; }
  let embedURL = ''; console.log(`[changeServer] Building URL: Server=${server}, Type=${type}, ID=${currentItem.id}`);
  if (type === 'movie') {
...
  } else if (type === 'tv') {
    const season = document.getElementById('season-select').value; const activeEpisodeItem = document.querySelector('#episode-list .episode-item.active');
    const episode = activeEpisodeItem ? activeEpisodeItem.dataset.episodeNumber : null;
    if (!season || !episode) { console.warn("TV show selected but season/episode not ready."); embedURL = `${baseURL}/${type}/${currentItem.id}`; }
    else { console.log(`[changeServer] Building TV URL: S=${season}, E=${episode}`);
...
    }
  }
  console.log("[changeServer] Setting iframe src to:", embedURL); const videoFrame = document.getElementById('modal-video'); if (videoFrame.src !== embedURL) { videoFrame.src = embedURL; }
}
